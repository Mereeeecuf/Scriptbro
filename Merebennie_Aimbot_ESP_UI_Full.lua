--[[
Full Delta Mobile Compatible Aimlock + ESP + UI LocalScript
Features:
- Intro watermark "Made by Merebennie" (top-left) with shadow, outline, gradient, pop animation + intro sound (rbxassetid://17208361335)
- Uses marketplace circle image (rbxassetid://80133146494844)
- Instance-based UI (no Drawing API) compatible with Delta Mobile Executor and PC
- Aimlock: smooth aimlock to nearest player inside circle, swipe override, auto-switch, adjustable speed/tilt/target part, filter friends
- FOV circle support: adjustable FOV size (20-400 px) and toggle to show/hide FOV ring
- Draggable Aimbot toggle button (tap to toggle, double-tap to move to nearest spawn), keybind V on PC
- Settings menu: sliders (aim speed, tilt, FOV size), dropdown (target part), toggles (filter friends, ESP, show FOV), spawn placement (place/red tween), UI animations and click sounds
- ESP: BillboardGui with name & distance
- All UI inside a ScreenGui; single LocalScript; cleanly commented
Defaults: Circle radius = 120px, Smoothness = 0.18, Target = "Head"
Made for Delta Mobile Executor compatibility.
Author: Merebennie (assembled for you)
--]]
...
do
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if dragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
            ...
            fovFrame.Position = UDim2.new(0, newPos.X + circle.AbsoluteSize.X/2, 0, newPos.Y + circle.AbsoluteSize.Y/2)
            fovShadow.Position = UDim2.new(0, newPos.X + circle.AbsoluteSize.X/2, 0, newPos.Y + circle.AbsoluteSize.Y/2 + 6)
        end
    end)
...
toggleButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(toggleButton, TweenInfo.new(0.08, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(orig.X.Scale, orig.X.Offset*1.05, orig.Y.Scale, orig.Y.Offset*1.05)}):Play()
...
addLabel(settingsFrame, "Ukuran FOV (px)", 184)
...
local function createOrUpdateBillboard(player)
    ...
    local bb = Instance.new("BillboardGui"); bb.Name = "MereESP"; bb.Adornee = hrp; bb.Size = UDim2.new(0,140,0,40); bb.AlwaysOnTop = true; bb.Parent = player.Character
