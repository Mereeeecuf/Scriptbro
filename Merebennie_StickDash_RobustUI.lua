...
-- UI overhaul: robust GUI parenting for many executors (KRNL, Synapse, Delta, CODEX)
-- NOTE: I did NOT change game logic/timings — only improved UI parenting and visuals.
...
local lastTrigger = 0
local TRIGGER_COOLDOWN = 0.35 -- changed to 0.35 per request
...
-- === Robust GUI creation & parenting (tries multiple executor-friendly parents) ===
local function safeParentGui(screenGui)
    -- try syn.protect_gui if available (Synapse / compatible)
    local protected = false
    if syn and type(syn.protect_gui) == "function" then
        pcall(function() syn.protect_gui(screenGui); protected = true end)
    elseif protect_gui and type(protect_gui) == "function" then
        pcall(function() protect_gui(screenGui); protected = true end)
    end

    -- order of attempts (best effort):
    -- 1) gethui()
    -- 2) get_hidden_gui()
    -- 3) LocalPlayer:WaitForChild("PlayerGui")
    -- 4) game:GetService("CoreGui") (last resort)
    local tryOrder = {
        function()
        ...
        function()
            local ok, playerGui = pcall(function() return LocalPlayer:FindFirstChild("PlayerGui") end)
    ...
    for _, fn in ipairs(tryOrder) do
        local ok, candidate = pcall(fn)
        if ok and candidate then
            -- final attempt to parent (some envs will error; so pcall)
            local s, e = pcall(function() screenGui.Parent = candidate end)
            if s then
    ...
    -- last fallback: try setting Parent directly to PlayerGui (again) with pcall
    pcall(function() screenGui.Parent = LocalPlayer:FindFirstChild("PlayerGui") end)
    return screenGui.Parent ~= nil, screenGui.Parent
end
...
-- Attempt safe parenting
local ok, parent = safeParentGui(ScreenGui)
if not ok then
    -- If all fails, try Parent=CoreGui as last ditch (pcall)
    pcall(function() ScreenGui.Parent = game:GetService("CoreGui") end)
end

-- === Polished UI (Container & elements) ===
...
ReopenBtn.Position = UDim2.new(0.01, 0, 0.78, 0)
